# PowerShell скрипт для добавления SSH ключа на сервер
# Использование: .\add-ssh-key.ps1 user@server

param(
    [Parameter(Mandatory=$true)]
    [string]$Server
)

$SSH_KEY_FILE = "ssh-keys/authorized_key.pub"
$SSH_KEY = "ssh-rsa  user@users-MacBook-Pro.local"

Write-Host "🔑 Добавляю SSH ключ на сервер $Server..." -ForegroundColor Yellow

# Проверяем, существует ли файл с ключом
if (Test-Path $SSH_KEY_FILE) {
    $SSH_KEY = Get-Content $SSH_KEY_FILE -Raw
    Write-Host "✓ Используется ключ из файла $SSH_KEY_FILE" -ForegroundColor Green
}

# Добавляем ключ на сервер
$command = "mkdir -p ~/.ssh && chmod 700 ~/.ssh && echo '$SSH_KEY' >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys && echo '✓ SSH ключ успешно добавлен!'"
ssh $Server $command

Write-Host ""
Write-Host "✅ SSH ключ успешно добавлен на сервер $Server" -ForegroundColor Green
Write-Host ""
Write-Host "Теперь вы можете подключаться к серверу без пароля:" -ForegroundColor Cyan
Write-Host "  ssh $Server" -ForegroundColor Yellow


