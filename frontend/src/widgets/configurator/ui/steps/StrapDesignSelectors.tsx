...
import styles from './StrapDesignSelectors.module.css'

interface StrapDesignSelectorsProps {
	params: any
	selectedStrapModel: any
}

export const StrapDesignSelectors = observer(function StrapDesignSelectors({ params, selectedStrapModel }: StrapDesignSelectorsProps) {
...
interface StrapDesignSelectorProps {
	paramTitle: string
	paramName: string
	colors: any[]
...
const StrapDesignSelector = observer(function StrapDesignSelector({
	paramTitle,
	paramName,
	colors,
	func,
	hasButterflyBuckle
}: StrapDesignSelectorProps) {
	...
	return (
		<div className={styles.selector}>
			<div className={styles.selectorContent}>
				<div 
					className={styles.selectorColorSwatch}
					style={{ backgroundColor: selectedColor?.color_code || '#e9e9e9' }}
				/>
				<span className={styles.selectorText}>{paramTitle}</span>
				<svg className={styles.selectorArrow} viewBox="0 0 12 7" fill="none">
			...
			<select
				...
				value={selectedColor?.color_title || ''}
			...
			>
				...
				{colors.map((color, idx) => (
					<option key={idx} value={color.color_title}>
						{color.color_title}
					</option>
				))}
			</select>
